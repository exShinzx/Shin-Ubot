...
try:
    ...
    from Shin.fns.tools import safe_load
try:
...
@Shin_cmd(pattern="bash", fullsudo=False, only_devs=False)
async def _(event):
...
@Shin_cmd(pattern="eval", fullsudo=False, only_devs=False)
async def _(event):
    try:
        cmd = event.text.split(maxsplit=1)[1]
    except IndexError:
    ...
    if any(item in cmd for item in KEEP_SAFE().All) and (
        not (event.out or event.sender_id == Shin_bot.uid)
    ):
...
@Shin_cmd(pattern="cpp", only_devs=False)
async def doie(e):
    ...
    open("cpp-Shin.cpp", "w").write(match)
    m = await bash("g++ -o CppShin cpp-Shin.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-Shin.cpp")
            if os.path.exists("CppShin"):
                os.remove("CppShin")
