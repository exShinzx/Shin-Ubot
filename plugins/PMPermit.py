...
from Shin.dB import DEVLIST
from Shin.dB.logusers_db import *
from Shin.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await Shin_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await Shin_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await Shin_bot.inline_query(
                        ...
                        _to_delete[user.id] = await Shin_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await Shin_bot.send_message(
                            user.id, message_
                        )
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await Shin_bot(BlockRequest(user.id))
                await Shin_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @Shin_cmd(pattern="(start|stop|clear)archive$", fullsudo=True)
    async def _(e):
    ...
    @Shin_cmd(pattern="(a|ok)(?: |$)", fullsudo=True)
    ...
    @Shin_cmd(pattern="(no|tolak)(?: |$)", fullsudo=True)
...
@Shin_cmd(pattern="block( (.*)|$)", fullsudo=True)
...
@Shin_cmd(pattern="unblock( (.*)|$)", fullsudo=True)
...
@Shin_cmd(pattern="listapproved$", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await Shin_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await Shin_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await Shin_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await Shin_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await Shin_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await Shin_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await Shin_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await Shin_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
        ...
        if res:
            res = [
                await event.builder.document(
                    ...
                    description="[ᴋᴀᴢᴜ](https://t.me/disiniShin)",
                    text=msg_,
                    buttons=buttons,
                    link_preview=False,
                )
    ...
    await event.answer(res, switch_pm="• Shin •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[Shin_bot.uid])
