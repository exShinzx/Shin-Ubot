...
from Shin.dB.blacklist_db import (
    add_blacklist,
    get_blacklist,
    list_blacklist,
    rem_blacklist,
...
from . import Shin_bot, Shin_cmd, events, get_string, udB


@Shin_cmd(pattern="blacklist( (.*)|$)", admins_only=True)
async def af(e):
    ...
    Shin_bot.add_handler(blacklist, events.NewMessage(incoming=True))
    await e.eor(get_string("blk_2").format(wrd))


@Shin_cmd(pattern="remblacklist( (.*)|$)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1).strip()
    chat = e.chat_id
    if not wrd:
...
@Shin_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
