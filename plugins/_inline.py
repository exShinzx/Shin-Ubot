...
from Shin._misc._assistant import callback, in_pattern
from Shin.dB._core import HELP, LIST
from Shin.fns.helper import gen_chlog, time_formatter, updater
from Shin.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/exShinzx/Shin-Ubot"),
        Button.url("• Support •", url="t.me/shinsupport"),
    ],
]
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Shin Help Menu", text=text, buttons=_main_help_menu
        )
...
async def help_func(ayra):
    ...
    text = _strings.get(key, "").format(Shin, len(HELP.get(key)))
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("Shin_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="Shin_updates.txt",
            buttons=[
                [Button.inline("• Uᴘᴅᴀᴛᴇ Nᴏᴡ •", data="updatenow")],
                [Button.inline("«", data="ownr")],
            ],
        )
        remove("Shin_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Shin Op", text=txt, link_preview=False, buttons=btn)
    ]
