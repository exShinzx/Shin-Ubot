...
from Shin.dB.gban_mute_db import (
    gban,
    gmute,
    is_gbanned,
    is_gmuted,
...
from Shin.dB.gcast_blacklist_db import (
    add_gblacklist,
    is_gblacklisted,
    rem_gblacklist,
)
from Shin.fns.tools import create_tl_btn, format_btn, get_msg_button
...
@Shin_cmd(pattern="gpromote( (.*)|$)", fullsudo=True)
async def _(e):
...
@Shin_cmd(pattern="gdemote( (.*)|$)", fullsudo=True)
async def _(e):
    x = e.pattern_match.group(1).strip()
    Shin_bot = e.client
    if not x:
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await Shin_bot.get_entity(user)
        except BaseException:
            return await e.eor(f"`Tidak Ada Pengguna Ditemukan Terkait {user}`", time=5)
        ...
        rank = "Not AdMin"
        c = 0
        async for x in Shin_bot.iter_dialogs():
            if (
                "group" in key.lower()
                and x.is_group
                or "group" not in key.lower()
...
@Shin_cmd(pattern="ungban( (.*)|$)", fullsudo=True)
async def _(e):
...
@Shin_cmd(pattern="gban( (.*)|$)", fullsudo=True)
async def _(e):
...
@Shin_cmd(pattern="g(admin|)cast( (.*)|$)", fullsudo=False)
async def gcast(event):
    ...
    if err != "":
        ...
        text += f"\Anda dapat melakukan `{HNDLR}Shin gcast-error.log` untuk mengetahui laporan kesalahan."
    await kk.edit(text)


@Shin_cmd(pattern="gucast( (.*)|$)", fullsudo=True)
...
@Shin_cmd(pattern="gkick( (.*)|$)", fullsudo=True)
async def gkick(e):
...
@Shin_cmd(pattern="gmute( (.*)|$)", fullsudo=True)
async def _(e):
...
@Shin_cmd(pattern="ungmute( (.*)|$)", fullsudo=True)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(Shin_bot.me)}",
        )
...
@Shin_cmd(pattern="gblacklist$")
async def blacklist_(event):
    await gblacker(event, "add")


@Shin_cmd(pattern="ungblacklist$")
