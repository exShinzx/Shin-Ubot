...
from Shin.dB import stickers
from Shin.dB.forcesub_db import get_forcesetting
from Shin.dB.gban_mute_db import is_gbanned
from Shin.dB.greetings_db import get_goodbye, get_welcome, must_thank
from Shin.dB.nsfw_db import is_profan
from Shin.fns.helper import inline_mention
from Shin.fns.tools import async_searcher, create_tl_btn, get_chatbot_reply

try:
    from ProfanityDetector import detector
except ImportError:
    detector = None
from . import LOG_CHANNEL, LOGS, asst, Shin_bot, get_string, types, udB
from ._inline import something


@Shin_bot.on(events.ChatAction())
...
async def DummyHandler(ayra):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await Shin_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await Shin_bot.edit_permissions(
                    ayra.chat_id, user.id, send_messages=False
                )
                res = await Shin_bot.inline_query(
    ...
    if ayra.user_joined or ayra.added_by:
        ...
        # gbans and @Shin checks
        if udB.get_key("Shin_BANS"):
            try:
                is_banned = await async_searcher(
                    "https://bans.Shin/api/status",
                    json={"userId": user.id},
                    post=True,
                    re_json=True,
                )
                if is_banned["is_banned"]:
                    ...
                    await ayra.client.send_message(
                        chat.id,
                        f'**@ShinBan:** Banned user detected and banned!\n`{str(is_banned)}`.\nBan reason: {is_banned["reason"]}',
                    )
...
@Shin_bot.on(events.NewMessage(incoming=True))
...
@Shin_bot.on(events.Raw(types.UpdateUserName))
