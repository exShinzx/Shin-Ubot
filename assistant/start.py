...
from Shin._misc import SUDO_M, owner_and_sudos
from Shin.dB.asst_fns import *
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(Shin_bot.me)
    )
    if custom_info:
        msg += "\n\n• Powered by **@Shinsupportgrp**"
    await event.edit(
        msg,
...
async def ayra(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey there {mention}, this is Shin Assistant of {me}!\n\n{ok}",
