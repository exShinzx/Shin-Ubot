...
try:
    from Shin.fns.gDrive import GDriveManager
except ImportError:
    GDriveManager = None
...
from Shin.fns.helper import fast_download, progress
from Shin.fns.tools import Carbon, async_searcher, get_paste, telegraph_client
from Shin.startup.loader import Loader
...
_buttons = {
    "otvars": {
        "text": "Variabel lain untuk ditetapkan @Shinsupportgrp:",
        "buttons": [
            [
...
async def update(eve):
    ...
    if heroku_api:
        ...
        execl(sys.executable, sys.executable, "-m", "Shin")
    ...
    if not match:
        try:
            ...
            return await okk.edit(
                f"**• Shin Ubot •**{cli}", file=img, buttons=button
            )
        except Exception as er:
            LOGS.exception(er)
    ...
    async with asst.conversation(e.sender_id) as conv:
        ...
        if GDrive._create_token_file(code=code.text):
            await conv.send_message(
                "`Berhasil!\nAnda siap menggunakan Google Drive Shin Ubot.`",
