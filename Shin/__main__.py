...
def main():
    ...
    if (
        ...
        and Shin_bot.run_in_loop(updater())
    ):
        Shin_bot.run_in_loop(bash("bash installer.sh"))

        os.execl(sys.executable, "python3", "-m", "Shin")

    Shin_bot.run_in_loop(startup_stuff())

    Shin_bot.me.phone = None

    if not Shin_bot.me.bot:
        udB.set_key("OWNER_ID", Shin_bot.uid)

    LOGS.info("Initialising...")

    Shin_bot.run_in_loop(autopilot())
    ...
    Shin_bot.run_in_loop(customize())

    # Load Addons from Plugin Channels.
    if plugin_channels:
        Shin_bot.run_in_loop(plug(plugin_channels))

    # Send/Ignore Deploy Message..
    if not udB.get_key("LOG_OFF"):
        Shin_bot.run_in_loop(ready())

    # Edit Restarting Message (if It's restarting)
    Shin_bot.run_in_loop(WasItRestart(udB))
