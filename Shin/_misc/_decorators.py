...
from ..version import Shin_version as Shin_ver
...
def Shin_cmd(
    pattern=None, unsafe_pattern=r"^[^/!#@\$A-Za-z]", incoming=True, from_users=DEVLIST, manager=False, Shin_bot=Shin_bot, asst=asst, **kwargs
):
    ...
    def decor(dec):
        async def wrapp(ay):
            ...
            try:
                ...
                await Shin_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await Shin_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                await asst.send_message(
                    ...
                    buttons=[
                        ...
                        Button.url(
                            "Repl",
                            "https://replit.com/@Shin/ShinStringSession",
                        ),
                    ],
                )
                sys.exit()
                ...
                ftext = "**Shin Client Error:** `Forward this to` @shinsupport\n\n"
                ftext += "**Shin Version:** `" + str(pyver)
                ftext += "`\n**Userbot Version:** `" + str(Shin_ver)
                ftext += "`\n**Telethon Version:** `" + str(telever)
                ftext += f"`\n**Hosted At:** `{HOSTED_ON}`\n\n"
                ftext += "--------START Shin CRASH LOG--------"
                ftext += "\n**Date:** `" + date
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Shin Client Error:** `Forward this to` @shinsupport\n\n",
                        )
                else:
